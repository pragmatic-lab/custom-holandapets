...
class AccountInvoiceSummaryReport(models.TransientModel):
			...
			for index in range(0, 1):
				...
				if data_new:
					for index_data in data_new:
						...
						sum_discount += index_data['discount']
	...
	def generate_body_excel(self, worksheet, user_id, model_spai, header_format, letter_black, bold_total, letter_black_name, bold_total_color, bold_total_gray, format_diag, header_format_subtitle):
		"""
		...
		for x in model_spai.search([('user_id', '=', user_id)]):
		...
		if columns_taxes_out_invoice and columns_categories_out_invoice:
			...
			for x in columns_taxes_out_invoice:
				...
				worksheet.write(row+1,col_fijo+1, 'DCTO / DEV', header_format_subtitle)
				worksheet.write(row+1,col_fijo+2, 'NETO', header_format_subtitle)
				col_fijo+=3
				...
				for index in range(0, size_column_taxes):
					...
					if data_new:
						for index_data in data_new:
							...
							worksheet.write(row,col_data+1, (index_data['discount']) or 0, bold_total)
	...
	def generate_data_complete_excel(self, worksheet, name_report, user_id, user_name, model_spai, bold, header_format, format_tittle, letter_black, bold_total, letter_black_name, bold_total_color, bold_total_gray, format_diag, header_format_subtitle):
		"""
		...
		self.generate_body_excel(worksheet, user_id, model_spai, header_format, letter_black, bold_total, letter_black_name, bold_total_color, bold_total_gray, format_diag, header_format_subtitle)
	...
	def generate_excel(self):
		...
		for x in data_user:
			...
			self.generate_data_complete_excel(worksheet, name_report, x['user_id'], x['name'], model_spai, bold, header_format, format_tittle, letter_black, bold_total, letter_black_name, bold_total_color, bold_total_gray, format_diag, header_format_subtitle)
