...
class ResUsers(models.Model):
    ...
    allow_refund = fields.Boolean('Allow refunds', default=False)
    allow_manual_customer_selecting = fields.Boolean('Allow manual customer selecting', default=True)
    is_restaurant_installed = fields.Boolean(compute='_compute_is_restaurant_installed')

    def _compute_is_restaurant_installed(self):
        for r in self:
