...
"use strict";
    ...
    models.load_models({
    	model: 'res.users',
    	fields: ['pos_security_pin',],
    	domain: function(self){ return [['id','=', 2]]; },
          	loaded: function(self, user){
          		if (user){
          			self.user_admin = user[0];	
          		}
          	},
    });
    ...
    chrome.DebugWidget.include({
    	start: function(){
    		var self = this;
            this._super();
            if (self.pos.user_admin && self.pos.user_admin.pos_security_pin){
            	this.$('.button.delete_orders').off("click").click(function(){
                	self.gui.ask_password(self.pos.user_admin.pos_security_pin).then(function(){
            	...
            	this.$('.button.delete_unpaid_orders').off("click").click(function(){
                	self.gui.ask_password(self.pos.user_admin.pos_security_pin).then(function(){
                		self.gui.show_popup('confirm',{
            	...
            	this.$('.button.export_unpaid_orders').off("click").click(function(){
                	self.gui.ask_password(self.pos.user_admin.pos_security_pin).then(function(){
            	...
            	this.$('.button.export_paid_orders').off("click").click(function(){
                	self.gui.ask_password(self.pos.user_admin.pos_security_pin).then(function(){
                		self.gui.prepare_download_link(
