...
class VATReportCompute(models.TransientModel):
	...
	@api.multi
	def compute_data_for_report_(self, user_id):
		self.ensure_one()
		# Compute report data
		if self.based_on == 'taxtags':
			self._inject_taxtags_values_user(user_id)
			self._inject_tax_taxtags_values_user(user_id)
		elif self.based_on == 'taxgroups':
			self._inject_taxgroups_values_user(user_id)
			self._inject_tax_taxgroups_values_user(user_id)
		# Refresh cache because all data are computed with SQL requests
		self.refresh()

	def _inject_taxtags_values_user(self, user_id):
	...
	taxtags AS
		(SELECT coalesce(regexp_replace(tag.name,
			...
			WHERE tag.id is not null AND movetax.tax_exigible
				AND move.company_id = %s AND move.create_date >= %s
				AND move.create_date <= %s AND move.state = 'posted'
				AND move.create_uid = %s
				AND move.date = '2020-05-08'
			GROUP BY tag.id, move.create_uid
...
FROM
	taxtags tag
		"""
		query_inject_taxtags_params = (self.company_id.id, self.date_from, self.date_to, user_id, self.id, self.env.uid)
		_logger.info(query_inject_taxtags% query_inject_taxtags_params)
		self.env.cr.execute(query_inject_taxtags, query_inject_taxtags_params)

	def _inject_taxgroups_values_user(self, user_id):


		"""Inject report values for report_vat_report_taxtags."""


		_logger.info('hgghfsghfshs')
		_logger.info('two')
		...
		query_inject_taxgroups = """
								...
								AND move.create_uid = %s
								AND move.date = '2020-05-08'
								GROUP BY taxgroup.id, move.create_uid
								...
								taxgroups groups
		"""
		query_inject_taxgroups_params = (self.company_id.id, self.date_from,self.date_to, user_id, self.id, self.env.uid, )
		_logger.info(query_inject_taxgroups%query_inject_taxgroups_params)
		self.env.cr.execute(query_inject_taxgroups,query_inject_taxgroups_params)

	def _inject_tax_taxtags_values_user(self, user_id):
	...
	taxtags_tax AS
		(
			...
			WHERE tag.id is not null AND movetax.tax_exigible
				...
				AND move.create_date >= %s AND move.create_date <= %s
				AND move.state = 'posted'
				AND move.create_uid = %s
				AND move.date = '2020-05-08'
			GROUP BY tag.id, tax.id, move.create_uid
			ORDER BY tax.name
		)
INSERT INTO
...
FROM
	taxtags_tax tt
		"""
		query_inject_tax_params = (self.id, self.company_id.id, self.date_from, self.date_to, user_id, self.env.uid)
		_logger.info(query_inject_tax%query_inject_tax_params)
		self.env.cr.execute(query_inject_tax, query_inject_tax_params)

	def _inject_tax_taxgroups_values_user(self, user_id):
		""" Inject report values for report_vat_report_tax. """
		# pylint: disable=sql-injection
		_logger.info('four')
		query_inject_tax = """
	...
	taxtags_tax AS
		(
			...
			WHERE taxtag.id is not null AND movetax.tax_exigible
				...
				AND move.create_uid = %s
				AND move.date = '2020-05-08'
			GROUP BY taxtag.id, tax.id, move.create_uid
			ORDER BY tax.name
		)
INSERT INTO
	report_vat_report_tax
