...
class import_bank_statement(models.TransientModel):
	_name = 'account.tax.report.company'
	...
	def get_account_tax_report(self):
		...
		row = 5
		at_qry = """select distinct tax_line_id from account_move_line where date BETWEEN '%s' AND '%s' AND company_id=%s and tax_line_id is not null""" % (
		...
		where aml.tax_line_id=%s AND aml.date BETWEEN '%s' AND '%s' AND aml.company_id=%s""" % (
					...
					for aml in qry_dict:
						...
						r_name_number = self.env['account.invoice'].search([('number', '=', r_name)])
		...
		return {
			...
			'url': '/web/binary/download_document?model=account.tax.report.company&field=datas&id=%s&filename=Account Tax Report from %s to %s.xlsx' % (
	...
	and at.type_tax_use='%s'""" % (
		...
		return {
			'type': 'ir.actions.act_url',
			'url': '/web/binary/download_document?model=account.tax.report.company&field=datas&id=%s&filename=Account Summary Report from %s to %s.xls' % (
			self.id, self.from_date, self.to_date),
			'target': 'self',
		}
